...
import { FileUser, Clock, Plus, X, ArrowRight, Check, Ban, Loader2, AlertCircle, BookX, TrendingUp } from 'lucide-react';
import { MeetingCard, statusColors } from '../../components/Dashboard/MeetingSection';
import { Link } from 'react-router-dom';
import { supabase } from '../../lib/supabase';
import { useBacklogSummary, useSemesterStatistics } from '../../hooks/useAcademicProgression';
...
const HODDashboard = () => {
    ...
    const { semesterStats, loading: semStatsLoading } = useSemesterStatistics();
        ...
        <div className="p-8 sm:p-12 space-y-10 bg-[#fcfdfe] min-h-screen">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    {/* NEW: Semester Performance Statistics */}
                    {semesterStats && semesterStats.length > 0 && (
                        <div className="bg-white rounded-[3rem] p-10 border-2 border-slate-100 shadow-sm relative overflow-hidden mt-8">
                            <div className="flex justify-between items-center mb-8">
                                <h2 className="text-2xl font-black text-[#1a1b4b] uppercase tracking-tighter flex items-center gap-3">
                                    <TrendingUp className="text-indigo-500" size={22} /> Batch Performance
                                </h2>
                            </div>
                            <div className="space-y-4">
                                {semesterStats.map(stat => (
                                    <div key={`${stat.academic_year}-${stat.semester_term}`} className="p-5 bg-slate-50/50 rounded-2xl border border-slate-100 flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-black text-[#1a1b4b] uppercase tracking-tight">{stat.academic_year} — Semester {stat.semester_term}</p>
                                            <p className="text-[12px] text-gray-400 font-bold mt-1">Students: {stat.total_students}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[14px] font-black text-[#1a1b4b] uppercase tracking-widest">{stat.pass_rate_percentage || 0}% Pass Rate</p>
                                            <p className="text-[11px] font-bold text-emerald-500 mt-1 uppercase">Avg SGPA: {stat.average_sgpa || 'N/A'}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
