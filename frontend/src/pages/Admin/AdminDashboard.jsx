        ...
        <div className="p-8 sm:p-12 space-y-10 bg-[#fcfdfe] min-h-screen">
            ...
            <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                    <h2 className="text-xl font-bold text-[#1a1b4b] flex items-center gap-2">
                       <Clock className="text-indigo-500" size={20} /> Latest System Activity
                    </h2>
                    <span className="text-[12px] font-semibold bg-indigo-50 text-indigo-500 px-3 py-1 rounded-full uppercase tracking-widest">Real-Time Sync</span>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {recentLogs.length > 0 ? recentLogs.map((log, i) => (
                        <div key={log.id} className="p-5 bg-gray-50/50 rounded-xl border border-gray-100 flex items-center justify-between hover:bg-white hover:shadow-md transition-all group">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center border border-gray-100 text-[#1a1b4b] font-bold group-hover:scale-110 transition-transform">
                                    {log.full_name?.charAt(0)}
                                </div>
                                <div>
                                    <p className="text-sm font-semibold text-[#1a1b4b]">Identity Update: {log.full_name}</p>
                                    <p className="text-[12px] text-gray-500 font-medium uppercase tracking-widest flex items-center gap-1 mt-0.5">
                                        Role: {log.role} • <span>{new Date(log.updated_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                    </p>
                                </div>
                            </div>
                            <span className="text-[12px] font-bold text-blue-500 bg-blue-50 px-2.5 py-1 rounded-md uppercase tracking-widest border border-blue-100 opacity-0 group-hover:opacity-100 transition-opacity">Trace</span>
                        </div>
                    )) : (
