...
import { usePromotionSummary, useDepartmentStatistics } from '../../hooks/useAcademicProgression';
...
const DeanDashboard = () => {
    ...
    const { deptStats, loading: deptLoading } = useDepartmentStatistics();
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 2. Mock Enrollment Trends (Number of students per month or semester)
            setEnrollmentData([
                { month: 'Jan', count: 45 },
                { month: 'Feb', count: 52 },
                { month: 'Mar', count: 48 },
                        ...
                        {deptStats.length > 0 ? deptStats.map((dept, i) => (
                            <div key={i} className="space-y-2 group/item">
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className="text-xs font-black text-[#1a1b4b] uppercase tracking-tight">{dept.department_name}</p>
                                        <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest">{dept.department_code}</p>
                                    </div>
                                    <span className="text-[12px] font-black text-[#1a1b4b] uppercase">CGPA: {dept.average_department_cgpa || 'N/A'}</span>
                                </div>
                                <div className="h-2 w-full bg-gray-50 rounded-full overflow-hidden">
                                    <div 
                                        className="h-full bg-[#1a1b4b] transition-all duration-1000 ease-out group-hover/item:bg-indigo-500" 
                                        style={{ width: `${(dept.average_department_cgpa / 10) * 100 || 0}%` }}
                                    ></div>
                                </div>
                            </div>
                        )) : (
