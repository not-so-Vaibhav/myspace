      ...
      <div className={`bg-white rounded-[1.75rem] overflow-hidden shadow-sm border border-gray-100 flex flex-col transition-all hover:shadow-md hover:-translate-y-0.5 ${past ? 'opacity-60 hover:opacity-100' : ''}`}>
        ...
        <div className="p-6 flex flex-col flex-1 gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            {isActive(ann) && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-red-50 text-red-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-red-100">
                <Megaphone size={10} /> Live
              </span>
            )}
            <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-50 text-gray-400 rounded-lg text-[9px] font-black uppercase tracking-widest border border-gray-100">
              <CalIcon size={10} /> {format(parseISO(ann.start_date), 'MMM dd')} – {format(parseISO(ann.end_date), 'MMM dd, yy')}
            </span>
            {isAdmin && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-indigo-100">
          ...
          {ann.attachment_url && !hasImage && (
            <a href={ann.attachment_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 mt-1 px-4 py-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-[#1a1b4b] rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors w-fit">
      ...
      <div className="bg-white rounded-[1.75rem] overflow-hidden shadow-sm border border-amber-100 flex flex-col transition-all hover:shadow-md group">
        ...
        <div className="bg-amber-50 px-6 py-2 border-b border-amber-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-black uppercase tracking-widest text-amber-600">Awaiting Approval</span>
          </div>
          {ann.submitted_by_name && (
            <span className="text-[9px] font-bold uppercase tracking-widest text-gray-400">By: {ann.submitted_by_name}</span>
          )}
        </div>
          ...
          {isAdmin && (
            <div className="flex gap-2 pt-4 border-t border-gray-50 mt-auto">
              <button 
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-colors shadow-sm"
              >
                <CheckCircle size={14} /> Approve
              </button>
              <button 
                onClick={() => handleReject(ann)} 
                className="flex-[0.5] flex items-center justify-center gap-2 py-2.5 bg-white border border-red-100 text-red-500 hover:bg-red-50 rounded-xl text-[9px] font-black uppercase tracking-widest transition-colors"
              >
    ...
    <div className="p-6 sm:p-10 space-y-8 bg-[#fcfdfe] min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        ...
        {(isAdmin || isFacultyLike) && !showForm && (
          <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-6 py-3 bg-[#1a1b4b] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#2d3a8c] transition-all shadow-md">
      ...
      {isAdmin && (
        <div className="flex gap-2 border-b border-gray-100 pb-0">
          <button
            ...
            className={`px-5 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-t-xl transition-colors border-b-2 -mb-px ${
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-t-xl transition-colors border-b-2 -mb-px ${
      ...
      {isFacultyLike && pendingList.length > 0 && (
        <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-amber-600 font-bold mt-0.5">They'll be published once an admin approves them.</p>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-8 max-w-3xl">
          ...
          {!isAdmin && (
            <div className="mb-5 p-3 bg-amber-50 border border-amber-100 rounded-xl flex items-center gap-2">
              ...
              <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest">This will be sent to admin for review before publishing</p>
            </div>
          )}
          <form onSubmit={handleCreate} className="space-y-5">
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Announcement Title</label>
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Attach Image or Document (Optional)</label>
              ...
              {form.file && <p className="text-[10px] font-bold text-emerald-600 mt-1">{form.file.name}</p>}
            </div>
            <div className="flex items-center gap-3 pt-2">
              <button type="submit" disabled={submitting} className="flex-1 flex items-center justify-center gap-2 bg-[#1a1b4b] text-white py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-[#2d3a8c] disabled:opacity-50 transition-colors">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="px-6 py-3.5 bg-gray-100 text-gray-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-gray-200">Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      {(!isAdmin || tab === 'active') && (
        <>
          ...
          {activeAnnouncements.length === 0 ? (
            ...
            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Active Notices</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
